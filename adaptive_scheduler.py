...
def parse_retry_after(value, default, now=None):
    """Return Retry-After as bounded seconds, accepting HTTP-date values."""
...
class AdaptiveScheduler:
    ...
    def __init__(self, max_concurrency=8, min_concurrency=1,
        ...
        self._pause_enabled = bool(pause_enabled)

    def configure(self, max_concurrency=None, proxies=None, browser_priority=None,
                  request_interval=None, pause_enabled=None):
        ...
        with self._condition:
            while True:
                ...
                if available and self._active < self.current_limit:
                    ...
                    base_interval = 0.75 if self._browser_priority else self._request_interval
                    # 拟人化：给请求间隔加随机抖动，避免固定节奏被风控识别
                    interval = base_interval * random.uniform(0.75, 1.35)
    ...
    def _finish(self, lease, status, error, retry_after):
        ...
        with self._condition:
            ...
            if not error and status is not None and 200 <= int(status) < 400:
                ...
                if self._pause_enabled and (risk or state.consecutive_failures >= self.failure_threshold):
                    multiplier = min(8, 2 ** max(0, state.consecutive_failures - self.failure_threshold))
                    cooldown = parse_retry_after(retry_after, self.base_cooldown) * multiplier
                    state.blocked_until = self._clock() + min(300.0, cooldown)
                    self._blocked_count += 1
                if self._pause_enabled and challenge:
                    self._site_blocked_until = max(
    ...
    def __call__(self, client, request=None, url=None, is_image=False, **kwargs):
        ...
        for _attempt in range(attempts):
            ...
            try:
                ...
                client.update_request_with_specify_domain(
                    request_kwargs,
