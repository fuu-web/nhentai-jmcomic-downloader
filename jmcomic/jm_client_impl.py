...
class AbstractJmClient(
    ...
    def request_with_retry(self,
        ...
        original_url = url
        last_domain_index = domain_index
        last_retry_count = retry_count
        for current_domain in range(domain_index, len(self.domain_list)):
            first_retry = retry_count if current_domain == domain_index else 0
            for current_retry in range(first_retry, self.retry_times + 1):
                request_url = original_url
                request_kwargs = dict(kwargs)
                if original_url.startswith('/'):
                    domain = self.domain_list[current_domain]
                    request_url = self.of_api_url(original_url, domain)
                    self.update_request_with_specify_domain(request_kwargs, domain, is_image)
                    jm_log(self.log_topic(), self.decode(request_url))
                elif is_image:
                    self.update_request_with_specify_domain(request_kwargs, None, is_image)

                if current_domain != 0 or current_retry != 0:
                    jm_log('req.retry', ', '.join([
                        f'次数: [{current_retry}/{self.retry_times}]',
                        f'域名: [{current_domain} of {self.domain_list}]',
                        f'路径: [{request_url}]',
                        f'参数: [{request_kwargs if "login" not in request_url else "#login_form#"}]',
                    ]))
                try:
                    resp = request(request_url, **request_kwargs)
                    return self.raise_if_resp_should_retry(resp, is_image)
                except Exception as e:
                    if self.retry_times == 0 and len(self.domain_list) == 1:
                        raise
                    self.before_retry(e, request_kwargs, current_retry, request_url)
                    last_domain_index = current_domain
                    last_retry_count = current_retry

        return self.fallback(request, original_url, last_domain_index + 1,
                             last_retry_count, is_image, **kwargs)
...
class JmHtmlClient(AbstractJmClient):
    ...
    def get_weekly_info(self):
        raise NotImplementedError('每周必看仅支持API模式')

    def get_weekly(self, category_id, type_id):
        raise NotImplementedError('每周必看仅支持API模式')
...
class JmApiClient(AbstractJmClient):
    ...
    def get_weekly_info(self):
        resp = self.req_api(self.API_WEEKLY)
        return resp.model_data

    def get_weekly(self, category_id, type_id):
        if not category_id or not type_id:
            raise ValueError('category_id and type_id must not be empty')
        resp = self.req_api(
            self.append_params_to_url(f'{self.API_WEEKLY}/filter', {
                'id': category_id,
                'type': type_id,
            })
        )
        ...
        def result(self):
            if not self.done:
                try:
                    ...
                    # 任务失败也清理缓存，否则该 id 在本进程内会被永久缓存为失败
