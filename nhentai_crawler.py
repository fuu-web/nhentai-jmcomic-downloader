...
import threading
from adaptive_scheduler import AdaptiveScheduler, endpoint_from_url, parse_proxy_pool
from utils import decode_utf8_response, repair_mojibake, sanitize_filename
from integrity import image_is_valid
...
class NHentaiCrawler:
    ...
    def __init__(self, proxy=None, config=None):
        ...
        self.scheduler = AdaptiveScheduler(
            ...
            initial_concurrency=min(4, self.config['max_workers']),
        )
        self.session = None
    ...
    def _adapt_page_response(self, scrapling_page, url):
        ...
        class AdaptedResponse:
            def __init__(self, page, url):
                self.page = page
                self.url = url
                self.status_code = getattr(page, 'status_code', getattr(page, 'status', 200))
                # Scrapling Page 通常有 .html 属性
                try:
                    self.text = page.html if hasattr(page, 'html') else str(page)
                    self.text = repair_mojibake(self.text)
                except Exception:
                    self.text = ''
    ...
    def download_image(self, url, save_path, retry=None):
        ...
        for attempt in range(retry):
            ...
            try:
                ...
                lease = self.scheduler.acquire([endpoint_from_url(url)])
                with req.Session() as dl_session:
                    ...
                    if lease.proxy:
                        dl_session.proxies = {'http': lease.proxy, 'https': lease.proxy}
                ...
                if partial_path.stat().st_size == 0:
                    raise IOError('响应为空')
                if not image_is_valid(partial_path):
                    raise IOError('图片损坏或数据不完整')
                partial_path.replace(save_path)
                lease.finish(status=response.status_code)
                return True
            except Exception as e:
    ...
    def download_gallery(self, gallery_id):
        ...
        for page_num, image_url in info['image_urls']:
            ...
            if image_path.exists() and image_is_valid(image_path):
        ...
        # 并发下载（仅处理不存在的图片）
        url_by_page = dict(info['image_urls'])
        download_tasks = [(page_num, url_by_page[page_num], path)
                          for page_num, path, exists in tasks if not exists]
        ...
        if len(downloaded) != info['num_pages']:
            logger.error(f'下载不完整: 成功 {len(downloaded)}/{info["num_pages"]}，保留临时文件供下次续传')
            return False

        # 打包成ZIP
        zip_name = f'{gallery_id}_{sanitize_filename(info["title"])}.zip'
        zip_path = Path(self.config['output_dir']) / zip_name
        partial_zip = zip_path.with_name(zip_path.name + '.part')
    ...
    def search_and_download(self, query, max_galleries=5):
        """搜索并下载"""
        url = 'https://nhentai.net/search/?' + urlencode({'q': query})
...
def main():
    ...
    parser.add_argument('-w', '--workers', type=int, choices=range(1, 65), default=3, help='并发下载数')
